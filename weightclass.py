...
class Theta(object):
    #THETA BASE REPRESENTATION IS A LONG ROW VECTOR
    ...
    def __init__(self, net_specs, initialization = 'random', row_entry = [0]):
        ...
        self.dimensions = Theta.get_mat_dims(self)
        self.count = Theta.num_weights(self)
        self.start_end_dict = Theta.start_end(self)

        #might be a more elegant way to do this
        if len(row_entry) == 1:
            self.row = Theta.initialize_theta(self, initialization)
        else:
            self.row = row_entry

    def initialize_theta(self, initialization, eps = 0.12):
    ...
    def __str__(self):
        string = 'Theta Matrices:\n'
        for matrix in self.dimensions:
            string += ('Theta ' + str(matrix) + ' shape: ' + str(Theta.get_matrix(self, matrix).shape) + '\n' + str(Theta.get_matrix(self, matrix)) + '\n\n')
        return string
    ...
    def __add__(self, other):
        ...
        return Theta(self.net_specs, row_entry = total)
